...
Today's automated suite tops out at 47.9% weighted statement coverage. Reads, writes, and error paths are validated against the in-process `go-proton-api` dev server, which diverges from production Proton in field ordering, header casing, key serialisation, and the entire custom-domain surface. Write tools (11 of 18) have no automated coverage; the only signal is the manual checklist at `docs/testing-checklist.md`. Token-rotation regression `5a99b02` has no guard test. CI cannot detect prod-API drift.
...
- `recorder.go` — thin wrapper around `gopkg.in/dnaeon/go-vcr.v4`. Exports `New(t *testing.T, name string) http.RoundTripper` and `Mode()` (reads `VCR_MODE`, default `replay`). Cassette path resolved relative to caller package: `testdata/cassettes/<pkg>/<name>.yaml`.
- `scrub.go` — `SaveHook` pipeline applied before any cassette is written to disk. Pipeline:
  ...
  - JSON body walker scrubs known sensitive keys to deterministic placeholders: `AccessToken`, `RefreshToken`, `UID`, `KeySalt`, `PrivateKey`, `Signature`, `Token`, `SrpSession`, `ServerProof`, `ClientProof`, `ClientEphemeral` → `REDACTED_<KEY>_<N>` where `<N>` is the running occurrence index per scenario.
  - Identifier rewrite: `RECORD_EMAIL` → `user@example.test`, `RECORD_DOMAIN` → `example.test`.
...
### `testdata/cassettes/<pkg>/<scenarioName>.yaml`
...
- Cassette metadata: `recorded_at`, `go_proton_api_version`, `mcp_version`, `scenario`. Populated by the record tool. Implementation detail (cassette `extensions` field vs sidecar `*.meta.json`) deferred to the plan; go-vcr v4's `Cassette` struct exposes the file directly, so either approach is open.
...
- Extract `main()` body into `run(ctx context.Context, args []string, env []string, stdin io.Reader, stdout, stderr io.Writer) int`. `main` becomes `os.Exit(run(...))`.
- CLI subcommands (`login`, `logout`, `status`, server default) read `PROTONMAIL_MCP_BASE_URL` from `env`; production omits it (uses `https://mail.proton.me`); tests inject the value plus cassette wiring.
...
- `make verify-cassettes` → runs `testvcr.lint` over `testdata/cassettes/**` and warns on stale metadata (>90 days or `go_proton_api_version` mismatch with `go.mod`).
...
```
*_test.go ─> run(ctx, args, env...) ─> session.New(BaseURL=env.PROTONMAIL_MCP_BASE_URL)
                                              │
                                              └─ Transport set to testvcr.New(t, cassette)
```
...
- `make verify-cassettes` warns when `recorded_at` is older than 90 days or `go_proton_api_version` mismatches `go.mod`. Warn-only by default; `STRICT=1` upgrades to error.
...
**Cassette inventory (target ~48 cassettes, ~2.4MB committed after scrub):**

- `tools/`: 18 tools × (happy + dominant error) = 36, plus 1 token-rotation, 1 SRP-login-with-2FA.
- `session/`: login (no-2FA), login (2FA), refresh, logout, expired-session retry. 5 cassettes.
- `cmd/`: login, logout, status (logged-in), status (logged-out). 4 cassettes.
- `server/`: boot + dispatch. 1 cassette.
...
| `internal/tools` | 32% | 92% | 36 cassette tests (happy + error) for all 18 tools |
...
- **HTTP/2 + go-vcr compatibility.** `go-proton-api` may negotiate HTTP/2 via the resty default transport. To be verified in the first implementation step by recording a single read scenario and replaying it; if HTTP/2 framing trips go-vcr v4, force HTTP/1.1 by supplying a transport with `ForceAttemptHTTP2: false` to both `proton.WithTransport` and `resty.SetTransport`.
