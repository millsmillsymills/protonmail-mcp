...
**`internal/session/appversion.go`:** Defines `appVersionHeader()` returning a Proton-acceptable `x-pm-appversion` value of the form `<api-os>-bridge@<version>` (e.g. `macos-bridge@3.24.1`). Live testing 2026-04-26 found Proton's API rejects every product name except `bridge` with code 2064 (`Product 'X' is not valid`); we send `bridge` and the latest published proton-bridge tag. `runtime.GOOS` is mapped: `darwin`→`macos`, `windows`→`windows`, anything else→`linux`. Both `proton.New` (via `proton.WithAppVersion`) and the raw client share this single helper so the two surfaces send identical headers. When Proton starts rejecting the pinned version (code 5002/5003), bump `bridgeAppVersion` to whatever `gh api repos/ProtonMail/proton-bridge/releases/latest -q .tag_name` returns.
